...
import getCertificationRequests from '@salesforce/apex/CMAChartDataLWCCtrl.getCertificationRequests';

export default class CertificationRequestsChart extends LightningElement {
    labels=[];
    details=[];
    colors=[];
    superLabel = `[`;
    ...
    constructor(){
        ...
        then(data=>{
            ...
            data.forEach(element => {
                let label = '';
                label =`"` +element.Name+ " - "+element.Status__c +" ("+element.Cnt+`)"`;
                let detail = '';
                detail = element.Cnt;
                let color = '';
